...
/* ================= ADD / WITHDRAW ================= */
export const addSavingsTransaction = async (req: Request, res: Response) => {
  try {
    ...
    const savingsBalance = result[0]?.balance || 0;

    if (transactionType === "withdraw" && amount > savingsBalance)
      return res.status(400).json({ message: "Insufficient savings balance" });

    if (transactionType === "add") {
      const current = await CurrentBalance.findOne({ userId });
      if (!current || current.currentBalance < amount)
        return res.status(400).json({ message: "Insufficient main balance" });
    }

    const transaction = await Savings.create({
      userId, bank, amount, transactionType, note
    });

    await CurrentBalance.findOneAndUpdate(
      { userId },
      { $inc: { currentBalance: transactionType === "add" ? -amount : amount } },
      { new: true }
    );
